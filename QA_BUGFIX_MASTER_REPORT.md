...
TC-057 (re-opened later in the pass once QA supplied the exact tracker row — see below) and TC-064 were originally left **BLOCKED** here — TC-064 still is, see the BLOCKED section below.

**TC-057 fix (added once the actual QA case was supplied — Module: Complaints, "Essential filters missing in the complaints list"):** the filter bar only ever had Search + Status, unlike the Leads list's fuller filter bar the QA case named as the comparison target. Added Assigned To (with an "Unassigned" option — `Complaint.assignedToUserId` is nullable, unlike Lead's), Department, Source, Website, Category (the originating public-form's subject code/label), Warranty Verification Status, and a Created From/To date range. Backend: 8 new optional fields on `QueryComplaintDto`, corresponding `where`-clause branches added to `buildFindAllQuery()` (shared by both `findAll()` and `exportToExcel()`, so filtering and export never drift apart), plus a new `GET /complaints/filter-options` endpoint that queries Prisma directly for the dropdown choices (departments, active users, in-use websites, in-use categories with labels) — deliberately not reusing `listDepartments()`/`listAssignableUsers()`/`listFormWebsites()`, since those are gated by `Department:View`/`Lead:View`/`FormConfiguration:View` respectively, none of which a Complaint viewer is guaranteed to hold; gating the new endpoint on `Complaint:View` only avoids a 403 on a permission a Complaint viewer has no reason to need. Frontend: `ComplaintFiltersBar.tsx` rebuilt to match `LeadFiltersBar.tsx`'s structural pattern, `api/complaints.ts` and `ComplaintList.tsx` (both the list fetch and the Excel export) wired through the new fields.

**Files changed:** `backend/src/complaints/complaints.service.ts`, `backend/src/complaints/complaints.controller.ts`, `backend/src/complaints/dto/create-complaint.dto.ts`, `backend/src/complaints/dto/query-complaint.dto.ts`, `backend/prisma/seed.ts`, `frontend/src/api/complaints.ts`, `frontend/src/components/complaints/ComplaintFiltersBar.tsx`, `frontend/src/components/complaints/complaintOptions.ts`, `frontend/src/pages/ComplaintList.tsx`, `frontend/src/pages/ComplaintDetails.tsx`, `frontend/src/pages/ComplaintForm.tsx`.

**Test performed:** `tsc --noEmit` both sides; standalone `tsc` check of `seed.ts`; for TC-057 specifically, `tsc --noEmit` + `nest build` (backend) and `tsc -b` + `vite build` (frontend) after the filter/endpoint/UI changes, plus a schema field-by-field cross-check confirming every new filter maps to a real, already-existing `Complaint` column (no schema change needed).

**Result:** PASS for TC-048/057/058/059/060/061/062/063/085. TC-064 BLOCKED (see below).
...
- **TC-057** (Group E) — **RESOLVED, no longer blocked.** Originally left blocked here because the specific repro text had been lost to this session's earlier context compaction. The user subsequently supplied the actual QA tracker row (Module: Complaints, "Essential filters missing in the complaints list"), and it was fixed — see the TC-057 write-up under Group E above.
- **TC-064** (Group E) — still BLOCKED. Flagged during the Complaints overhaul as requiring a decision the codebase gives no precedent for; the specific repro text for this case was lost to the same context compaction and has not yet been re-supplied. Re-confirm the exact requirement with QA before attempting a fix.
- **TC-091** (Group C) — still BLOCKED. This report previously stated TC-091's blocker was "no Fan Type field/requirement existed anywhere in the schema or PDF template" — that is a documentation error introduced while reconstructing this report after the compaction: that reasoning actually belongs to a *different* case, TC-086 (see TC-086 in the Standalone table above, since fixed). TC-091's own original repro text and blocker reasoning did not survive the compaction and have not been recovered; it is tracked under Group C's color-redesign work but its actual requirement is currently unknown. Re-confirm the exact requirement with QA before attempting a fix.
...
Net open items after this pass: **TC-064** (Group E) and **TC-091** (Group C) remain genuinely blocked pending clarification from whoever filed the original QA case, since the exact requirement text for these two specifically did not survive this session's context compaction and no other artifact in the codebase specifies it. TC-057, previously in this same position, was resolved once its actual QA tracker row was supplied (see above). Everything else originally flagged as an architecture gap was either fixed once properly scoped, or is listed above with its concrete blocker.
